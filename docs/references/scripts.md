...
| Change intent             | `pnpm change --bump <none\|patch\|minor\|major> --summary "<text>" @dragone/ui` |
| Change status             | `pnpm change status`                                                            |

`pnpm commit` and bare `pnpm change` (without `--bump`/`--summary` and package names) open
interactive prompts and will hang an agent; use the non-interactive forms above.
