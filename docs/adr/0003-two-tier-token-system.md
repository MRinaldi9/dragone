...
- Runtime tokens (e.g. `--drgn-runtime-select-width: var(--ngp-select-width)`) bridge ng-primitives dynamic CSS custom properties into the `--drgn-*` namespace. They must be declared **on the element that carries the inline `--ngp-*` value** (in the consuming component's CSS), never on `:root`: `var()` inside a custom property resolves at the declaring element, so a `:root`-level alias can never see element-scoped inline values and resolves to the guaranteed-invalid value, dropping the consuming declaration. Static values that ng-primitives never sets at runtime (e.g. `--ngp-shadow-lg`, which only exists in its example theme) are plain Component Tokens (`--drgn-shadow-lg`) with a concrete value, not runtime aliases.
